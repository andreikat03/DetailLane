...
self.addEventListener('activate', event => { event.waitUntil(caches.keys().then(keys => Promise.all(keys.filter(k => k !== CACHE && OWN_CACHE_PREFIXES.some(prefix => k.startsWith(prefix))).map(k => caches.delete(k))))); self.clients.claim(); });
